...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
		...
		self.opacity = 0.8f;
		self.color = [UIColor colorWithWhite:0.2f alpha:0.7f];
...
- (void)layoutSubviews {
    ...
    if ([detailsLabel.text isKindOfClass:[NSNull class]] || !detailsLabel.text) {
        detailsLabelSize = CGSizeMake(0, 0);
    }else{
        detailsLabelSize = MB_MULTILINE_TEXTSIZE(detailsLabel.text, detailsLabel.font, maxSize, detailsLabel.lineBreakMode);
    }
