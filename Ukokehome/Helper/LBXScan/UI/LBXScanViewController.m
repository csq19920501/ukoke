...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    if (!_style) {
        _libraryType = SLT_ZXing;
        _scanCodeType = SCT_QRCode;
        _isOpenInterestRect = YES;
        _style = [self InnerStyle];
    }
    ...
    [self requestCameraPemissionWithResult:^(BOOL granted) {
        ...
        if (granted) {
            ...
            [self showError:@"Please turn on the camera permissions of this program in Settings privacy"];
        }
    }];
...
- (LBXScanViewStyle*)InnerStyle
{
    ...
    style.notRecoginitonArea = RGB(110, 211, 238);
        ...
        case SLT_Native:
        {
            ...
            if (!_scanObj )
            {
                ...
                self.scanObj = [[LBXScanNative alloc]initWithPreView:videoView ObjectType:@[strCode] cropRect:cropRect success:^(NSArray<LBXScanResult *> *array) {
                    ...
                    [weakSelf scanResultWithArray:array isScan:YES];
                }];
        ...
        case SLT_ZXing:
        {
            ...
            if (!_zxingObj) {
                ...
                self.zxingObj = [[ZXingWrapper alloc]initWithPreView:videoView block:^(ZXBarcodeFormat barcodeFormat, NSString *str, UIImage *scanImg) {
                    ...
                    [weakSelf scanResultWithArray:@[result] isScan:YES];
        ...
        case SLT_ZBar:
        {
            ...
            if (!_zbarObj) {
                ...
                self.zbarObj = [[LBXZBarWrapper alloc]initWithPreView:videoView barCodeType:[self zbarTypeWithScanType:_scanCodeType] block:^(NSArray<LBXZbarResult *> *result) {
                    ...
                    [weakSelf scanResultWithArray:@[scanResult] isScan:YES];
                }];
            }
            [_zbarObj start];
#endif
...
- (void)scanResultWithArray:(NSArray<LBXScanResult*>*)array isScan:(BOOL)scan
{
    //设置了委托的处理
    if (_delegate) {
        [_delegate scanResultWithArray:array isScan:scan];
    }
    ...
    switch (_libraryType) {
        case SLT_Native:
        {
            ...
            if ([[[UIDevice currentDevice]systemVersion]floatValue] >= 8.0)
            {
                [LBXScanNative recognizeImage:image success:^(NSArray<LBXScanResult *> *array) {
                    [weakSelf scanResultWithArray:array isScan:NO];
                }];
            }
            else
            {
                [self showError:@"Native lower than ios8.0 system does not support recognition of image barcode"];
            }
#endif
        }
            break;
        case SLT_ZXing:
        {
            ...
            [ZXingWrapper recognizeImage:image block:^(ZXBarcodeFormat barcodeFormat, NSString *str) {
                ...
                [weakSelf scanResultWithArray:@[result] isScan:NO];
            }];
#endif
        ...
        case SLT_ZBar:
        {
            ...
            [LBXZBarWrapper recognizeImage:image block:^(NSArray<LBXZbarResult *> *result) {
                ...
                [weakSelf scanResultWithArray:@[scanResult] isScan:NO];
...
- (NSString*)nativeCodeWithType:(SCANCODETYPE)type
{
    switch (type) {
        ...
        case SCT_BarCode128:
            ...
            break;
        case SCT_BarCodeITF:
            return @"ITF bar code :only ZXing support";
            break;
        case SCT_BarEAN13:
